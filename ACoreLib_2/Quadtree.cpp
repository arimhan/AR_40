...
int	AQuadtree::GetLodType(ANode* pNode) 
{
	...
	if (pNode->m_pNeighborList[3] && pNode->m_pNeighborList[0]->m_iCurrentLod
...
int	 AQuadtree::UpdateIndexList(ANode* pNode, DWORD dwCurentIndex, DWORD dwNumLevel) 
{
	...
	DWORD dwYCell = 0; DWORD dwXCell = 0;
...
void AQuadtree::DrawDebugRender(ABox* pBox) 
{
	...
	//Box에서 돌린 VB 고대로~
	// +Z plane
	m_BoxDebug.m_VertexList[index].p = T::TVector3(1.0f, 1.0f, 1.0f);
