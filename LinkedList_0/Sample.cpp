//211207 3_LinkedList_0번 수정작업
//* _s버전
//* 5번 삭제 수정 : 6번 검색 후 삭제로 처리 
//* win32 -> x64에서 자잘한 오류 발생
//char* -> const char => 빌드 완료
//로드시 액세스 오류 발생
#include "Sample.h"

bool InsertLink(TNode* pFindNode, const char* pName, int m_iAge, const char* pAddr, int* pData)
{
...
void  LoadLink(const char* pLoadFileName)
{
	...
	printf("\n'save.txt'를 찾을 수 없습니다. (2)저장 후 이용해주세요!\n"); //	fp_dest = NULL;
	fclose(fp_dest);
	return;
}
void  NewLink(const char* pName, int m_iAge, const char* pAddr, int* pData)
{
	TNode* pNewData = new TNode;
	strcpy(pNewData->m_szName, pName);
	pNewData->m_iAge = m_iAge;
