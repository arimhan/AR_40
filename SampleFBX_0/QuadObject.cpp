...
#include "Input.h"
HRESULT AQuadObject::CreateConstantBuffer(ID3D11Device* pd3dDevice)
{
	HRESULT hr;
...
bool AQuadObject::SetBuffer(ID3D11Device* pd3dDevice)
{
	if (FAILED(CreateConstantBuffer(pd3dDevice)))
	{
		MessageBox(0, _T("CreateConstantBuffer  角菩"), _T("Fatal error"), MB_OK);
		return false;
	}
...
void AQuadObject::ComputeKernel(int iRadius )
{	
	float sigma = 1.0f;
	m_iMask = iRadius * 2 + 1;	
...
// 
//--------------------------------------------------------------------------------------
HRESULT AQuadObject::CreateTextures(ID3D11Device* pDevice, UINT iWidth, UINT iHeight)
{
	HRESULT hr = S_OK;
	...
	//溅捞歹 府家胶 积己 : 表捞 甘 溅档快俊辑 
	//desc format俊 蝶扼 贸府规过 盒府
	...
	if (srvDesc.Format == DXGI_FORMAT_R32_FLOAT || srvDesc.Format == DXGI_FORMAT_R24_UNORM_X8_TYPELESS)
	{
		...
		if (FAILED(hr = pDevice->CreateShaderResourceView(pDSTexture.Get(), &srvDesc, m_pDsvSRV.GetAddressOf()))) { return hr; }
	}
