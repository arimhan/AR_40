...
void AOctree::Frame(float time)
{
	...
	{ m_Player->m_Pos.x - vHalf.x, m_Player->m_Pos.y - vHalf.y,  m_Player->m_Pos.z + vHalf.z };
...
{
	m_Player = new AObject();
	m_vSize = { fMaxX, fMaxY, fMaxZ };
	m_RootNode = CreateNode(nullptr, AVector3(0, 0, 0), m_vSize);
	Buildtree(m_RootNode);
	AddObject(m_Player);
	return true;
}
void AOctree::Buildtree(ANode* pNode)
{
	if (pNode->m_Rect.vSize.x >= 30.0f && pNode->m_Rect.vSize.y >= 30.0f && pNode->m_Rect.vSize.z >= 30.0f)
	{
		AVector3 vHalf = pNode->m_Rect.vSize / 2.0f;

		pNode->m_Child[0] = CreateNode(pNode, pNode->m_Rect.vPivot, vHalf);
		Buildtree(pNode->m_Child[0]);

		pNode->m_Child[1] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x + vHalf.x,
			pNode->m_Rect.vPivot.y, pNode->m_Rect.vPivot.z), vHalf);
		Buildtree(pNode->m_Child[1]);

		pNode->m_Child[2] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x + vHalf.x,
			pNode->m_Rect.vPivot.y, pNode->m_Rect.vPivot.z - vHalf.z), vHalf);
		Buildtree(pNode->m_Child[2]);

		pNode->m_Child[3] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x,
			pNode->m_Rect.vPivot.y, pNode->m_Rect.vPivot.z - vHalf.z), vHalf);
		Buildtree(pNode->m_Child[3]);

		pNode->m_Child[4] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x,
			pNode->m_Rect.vPivot.y + vHalf.y, pNode->m_Rect.vPivot.z), vHalf);
		Buildtree(pNode->m_Child[4]);

		pNode->m_Child[5] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x + vHalf.x,
			pNode->m_Rect.vPivot.y + vHalf.y, pNode->m_Rect.vPivot.z), vHalf);
		Buildtree(pNode->m_Child[5]);

		pNode->m_Child[6] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x + vHalf.x,
			pNode->m_Rect.vPivot.y + vHalf.y, pNode->m_Rect.vPivot.z - vHalf.z), vHalf);
		Buildtree(pNode->m_Child[6]);

		pNode->m_Child[7] = CreateNode(pNode, AVector3(pNode->m_Rect.vPivot.x,
			pNode->m_Rect.vPivot.y + vHalf.y, pNode->m_Rect.vPivot.z - vHalf.z), vHalf);
		Buildtree(pNode->m_Child[7]);
	}
}
int AOctree::CheckRect(ANode* pNode, AObject* Obj)
{
...
ANode* AOctree::FindNode(ANode* pNode, AObject* pObj)
{
	do {
...
bool AOctree::AddObject(AObject* pObj)
{
	ANode* pFindANode = FindNode(m_RootNode, pObj);
	if (pFindANode != nullptr)
	{
		pObj->m_NodeIndex = m_ObjectList.size();
		m_ObjectList.insert(make_pair(pObj->m_NodeIndex, pObj));
		pFindANode->AddObject(pObj->m_NodeIndex);
		return true;
	}
	return false;
}
void AOctree::Release()
{
	delete m_RootNode;
	m_RootNode = nullptr;
...
ANode* AOctree::CreateNode(ANode* pParent, AVector3 vPivot, AVector3 vSize)
{
	ANode* pNode = new ANode(vPivot, vSize);
	if (pParent != nullptr)
	{
		pNode->m_Depth = pParent->m_Depth + 1;
		pNode->m_Parent = pParent;
	}
	pNode->m_Index = ANode::g_Counter;
	ANode::g_Counter++;
	return pNode;
}
AOctree::AOctree()
